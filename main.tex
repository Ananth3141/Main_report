...
where
\begin{equation*}
    \Lambda(r) = \diag(\lambda_1(r),…,\lambda_d(r))
\end{equation*}
...
\section{Student model and loss function}

We adopt a linear-in-$\bx$ student with general $r$-dependence:
\begin{equation}
...
The cost \eqref{eq:loss_explicit} can be expanded in matrix entries. Off-diagonal entries of $A(r)$ contribute only positive terms to the trace, while $\Sigma(r)^{-1}$ has no off-diagonal part. Hence setting off-diagonal entries of $A(r)$ to zero strictly reduces the cost without affecting the diagonal fit. Writing $A(r) = \diag(a_1(r), a_2(r), \ldots, a_d(r))$ with each $a_k(r)$ a degree-$(K-1)$ polynomial, the trace splits as
\[
\Tr[\cdots] \;=\; \sum_{k=1}^d \big(a_k(r) - 1/\sigma_k(r)\big)^2 \sigma_k(r).
\]

\paragraph{Non-critical directions ($k \geq 2$).} Here $\sigma_k(r) = 1$ and $1/\sigma_k(r) = 1$, so the $k$-th term is $(a_k(r) - 1)^2$. The polynomial fit $a_k(r) \equiv 1$ (achievable since the constant function belongs to the polynomial basis) gives zero loss. These directions are trivially learned.

\paragraph{Critical direction ($k = 1$).} Drop the subscript and call this $a(r) \equiv a_1(r)$. The relevant loss is
\begin{equation}
\mathcal{L}_1(a) \;=\; \int \rho_\Delta(r)\, \big(a(r) - 1/\sigma_1(r)\big)^2 \sigma_1(r)\, \rmd r.
\label{eq:L1}
\end{equation}

Define the \emph{target function} and \emph{weight function}:
\begin{equation}
...
Then
\begin{equation}
\mathcal{L}_1(a) \;=\; \int w(r)\, \big(a(r) - g(r)\big)^2\, dr.
\label{eq:L1_compact}
\end{equation}
This is a \emph{weighted least-squares polynomial fit} of $g$ on the support of $\rho_\Delta$ (i.e., on $|r-r_c| > \Delta/2$). Note that
\begin{itemize}
...
\label{eq:outer}
\end{equation}
Both are $\Delta$-independent at this level of approximation.
...
\end{equation}
The loss function in \eqref{eq:L1_compact} has been modified to accommodate finite number of data points P. 
...
where
\begin{equation*}
    \delta M = \frac{1}{P}\sum_{\mu=1}^{P} [w(r_\mu)\phi_\mu \phi_\mu^T - M];\hspace{2em}\delta b = \frac{1}{P}\sum_{\mu=1}^{P} [w(r_\mu)g(r_\mu)\phi_\mu - b]
\end{equation*}
...
and
\begin{equation}
    \mathbb{E}[\delta M \delta M^T] = \mathbb{E}[\delta b \delta b^T] = O(P^{-1})
\end{equation}
\begin{equation}
...
Then,
\begin{equation} 
    \hat\theta (\Delta,K,P) = (A + \delta M )^{-1} (b + \delta b) = \big (A^{-1} - A^{-1}\delta M A^{-1} + O(\delta M^2) \big )(b + \delta b)
\end{equation}
\begin{equation}
    \implies \hat\theta = A^{-1}b + A^{-1}\delta b - A^{-1}\delta M A^{-1}b + O(P^{-1})
\end{equation}
\begin{equation}
     \implies \hat\theta = \theta + A^{-1}\delta b - A^{-1}\delta M \theta + O(P^{-1})
\end{equation}
\begin{equation}
     \implies \hat \theta (\Delta, K, P) = \theta(\Delta,K) + \frac{1}{\sqrt{P}}\eta (\Delta, K) + O(P^{-1})
\end{equation}
where 
\begin{equation}
    \eta(\Delta, K) = \sqrt{P} (A^{-1}\delta b - A^{-1}\delta M\theta)
\end{equation}
The first and second moments of $\eta$ can be found to be,
\begin{equation} 
    \mathbb{E}[\eta]  = \sqrt{P} A^{-1} \qquad \mathbb{E}[\delta b] - A^{-1} \qquad \mathbb{E}[\delta M\theta] = 0
\end{equation}
\begin{split}
 \mathbb{E}[\eta\eta^T] &= \mathbb{E}[P(A^{-1}(\delta b - \delta M\theta))(A^{-1}(\delta b - \delta M\theta))^T]\\
& = PA^{-1}\mathbb{E}[\delta b \delta b^T - \delta b (\delta M \theta)^T - (\delta M\theta)\delta b^T + (\delta M\theta)(\delta M\theta)^T(A^{-1})^T\\
\end{split}
\intertext{The expectation evaluates to an $O(P^{-1})$ quantity. Therefore}
& \mathbb{E}[\eta\eta^T] = O(1)\\
\intertext{Now we can arrive at the first and second moments of $\hat\theta(\Delta, K, P)$ }
&\mathbb{E}[\hat\theta] = \theta(\Delta, K) + O(P^{-1})\\
\begin{split}
\mathbb{E}[\hat\theta \hat\theta^T] &= \mathbb{E}[\big (\theta(\Delta,K) + \frac{1}{\sqrt{P}}\eta(\Delta, K)+O(P^{-1})\big )\big (\theta(\Delta,K) + \frac{1}{\sqrt{P}}\eta(\Delta, K)+O(P^{-1})\big )^T]\\
&= \mathbb{E}[\theta\theta^T + \frac{1}{\sqrt{P}}(\eta\theta^T + \theta\eta^T)+\frac{1}{P}\eta\eta^T+O(P^{-3/2})] \\
&= \theta\theta^T + \frac{1}{P}\langle\eta\eta^T\rangle+O(P^{-3/2})\\
&= \theta\theta^T + O(P^{-1})\text{ ,to the leading order in P.}
\intertext{Now that we have the corrections $\theta(\Delta, K)$ with finite samples and since we already obtained the scaling of \theta(\Delta) we can then proceed to obtain the following:}
&\hat\theta(\Delta, K, P) = \big (\theta_0 + \Delta^{1-\nu}\theta_1 + O(\Delta^{3-\nu})\big ) + \frac{1}{\sqrt{P}}\eta(\Delta, K) \\
\end{split}
\end{flalign*}
%\end{equation*}
