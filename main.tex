...
\end{equation}
where $\Xi(\Delta, \nu) \in \mathbb{R}^{K\cross K}$ is zero-mean Gaussian matrix random variable with $\mathbb{E}[\Xi] = 0$.\\
...
\end{equation}    
\subsection{Trade-off Between Deterministic Gap Bias and Finite Data Statistical Noise}\label{subsec:three_case_scaling}
 From eqn.\ref{eq:cov_finite_scaling}, we can see that the effect of finite data affects the way that the covariance scales. To understand how finite-data fluctuations 
\begin{enumerate}
    \item \textbf{Sub-critical egime ($\nu < 1/2$):}
\end{enumerate}

\begin{equation}
    \implies \del M \sim O(P^{-1/2}),
\qquad
\del b \sim O(P^{-1/2})
\end{equation}
