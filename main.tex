...
One of the successes of modern Machine learning models is due to the generalization capability of the model \sgp{find \& cite different famous papers} where the model is able to accurately predict the solution to problems not in the training dataset. Although this capability is the selling point of several of the foundational models, the mechanism behind this generalization is less understood. Our goal in this work is to investigate this question in a toy setting where we look at the prediction error of the neural network in parameters not in the training dataset. We consider dynamical systems that follow the normal form of pitchfork, transcritical and Hopf bifurcation wherein the state variable undergoes a transition upon variation of the system parameter. The model is trained on data away from the critical point and the task it is intended to perform is to pred Besides these normal forms, we also test the generalizat

Understanding how generative models generalize near critical phenomena—such as bifurcations, phase transitions, and scale-invariant fluctuations—is a central challenge in machine learning for the physical sciences. In critical regimes, system dynamics are marked by diverging variance and persistent temporal memory (critical slowing down). This study evaluates the theoretical capacity of deep neural networks—specifically score-based generative models—to learn and generalize near critical points when trained exclusively using off-critical data.
\section{Linear Toy Model}
\label{sec:toy}

\subsection{Data-generating process}\label{subsec:generation}

% \sout{Let $r \in \R$ be a one-dimensional control parameter. We model a bifurcation at $r = r_c$ by introducing a conditional Gaussian distribution whose covariance becomes singular in one direction as $r \to r_c$.}Let $r \in \mathbb{R}^d$ denote a continuous state or control variable. We model a critical regime near a boundary or threshold $r_c$ by introducing a state-dependent covariance kernel whose spectrum contains a singular or power-law divergent component as $r \to r_c$. While such singular structures naturally emerge near dynamical bifurcations (via critical slowing down and spatial soft modes), this statistical formulation broadly encompasses any critical phenomenon characterized by scale-invariant fluctuations, manifold edge singularities, or ill-conditioned sampling measures.
...
\end{equation}
where the orthonormal columns of $Q(r) = [\be_1(r), \be_2(r),\ldots, \be_d(r)]$ define the \textbf{principal fluctuation directions(eigenvectors)} in state space $\mathbb{R}^d$, and $\lambda_k(r)$ represents the variance along direction $\be_k(r)$.

\paragraph{Diverging critical mode.} Near the \sout{bifurcation} critical point $r_c$, the system undergoes critical fluctuations where variance along the primary Order Parameter mode--- the critical direction $\be_1(r)$---diverges, while fluctuations along all transverse modes $\be_{k\geq 2}$ remain bounded.\\
Mathematically, as $r\to r_c$:
\begin{equation*}
...
\end{equation*}
Here, "direction" refers specifically to the principal eigenvector $\be_1$ corresponding to the dominant eigenvalue $\lambda_1(r)$. Along this eigenvector, variance grows parametrically faster than in any orthogonal subspace:
\begin{equation*}
...
\end{figure}
Without loss of generality, we choose our coordinate basis such that the critical direction coincides with the first canonical basis vector $\be_1 = (1, 0, \ldots, 0)^\top$. Therefore the covariance matrix simplifies to,
...
\]
As $r \to r_c$, the variance along $\be_1$ diverges as $|r-r_c|^{-\nu}$, mimicking the diverging fluctuations near a critical point. The critical exponent $\nu > 0$ governs the power-law divergence of variance along the order parameter mode, playing an analogous role to critical susceptibility exponents ($\gamma$) in continuous phase transitions or soft-mode instabilities in dynamical systems.
...
Assuming \eqref{eq:cond_dist} as the implicit teacher, we adopt a linear-in-$\bx$ student with general $r$-dependence:
\begin{equation}
s_{\btheta}(\bx, r) \;=\; -A(r) \bx, \qquad A(r) \in \R^{d \times d}.
\label{eq:student}
\end{equation}
We parameterize $A(r)$ as a degree-$(K-1)$ polynomial in $r$ in some basis $\{\phi_k(r)\}_{k=0}^{K-1}$ (monomials, Chebyshev, Legendre --- the choice does not affect the result). While any complete polynomial basis spans the same finite-dimensional function space $\mathbb{P}_{K-1}$ at the exact theoretical limit ($P \to \infty$), using orthogonal polynomials ensures well-conditioned normal matrices and rapid spectral convergence during optimization, avoiding the severe ill-conditioning associated with standard monomial bases. For concreteness:
\[
...
\begin{equation}
    \mathcal{L}(a) \;=\; \int \rho_{\Delta}(r)\Big(a(r)- \frac{1}{\sigma(r)}\Big)^2\sigma(r)\,\rmd r
    \label{eq:L1}
\end{equation}
...
\label{eq:L1_compact}
\end{equation}
of $g$ on the support of $\rho_\Delta$ (i.e., on $|r-r_c| > \Delta/2$). \sgp{Expand this section.}\ant{Not sure how exactly to expand} Note that
\begin{itemize}
...
\begin{tcolorbox}[
    ...
    title=\textbf{Ridge Regression Explained},
...
\textbf{Physical Toy Example: Order Parameter Extraction near Criticality}
